...
def main():
                ...
                for p2 in p2s:
                    for p3 in p3s:
                        ...
                        for i in range(epochs):
                            ...
                            # BF.append(y)
                            print(y)
                            BF.append([('%f' % y), xoverChoice, mutationChoice, mix, "-", p2, p3])
    ...
    my_df = pd.DataFrame(BF)
    my_df.to_csv('csv/output-'+str("%03d" % savenum)+'.csv', index=False, header=['BF', 'xover', 'mutation', 'mixingFactor', 'p1', 'p2', 'p3'])
